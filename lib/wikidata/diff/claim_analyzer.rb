    ...
    if parent_content.nil?
      current_content_claims.each do |claim_key, current_claims|
        current_claims.each_with_index do |current_claim, index|
          ...
          added_references = ReferenceAnalyzer.reference_updates(current_claim, added_references, claim_key,
                                                                 index)
          added_qualifiers = QualifierAnalyzer.qualifier_updates(current_claim, added_qualifiers, claim_key,
                                                                 index)
        end
      end
    else
        ...
        if parent_content_claims.key?(claim_key)
          ...
          current_claims.each_with_index do |current_claim, index|
            ...
            if parent_claim.nil?
              ...
              added_claims << { key: claim_key, index: index }
              # check if there's any references or qualifiers in this claim
              added_references = ReferenceAnalyzer.reference_updates(current_claim, added_references,
                                                                     claim_key, index)
              added_qualifiers = QualifierAnalyzer.qualifier_updates(current_claim, added_qualifiers,
                                                                     claim_key, index)
              ...
              # check if there's any references or qualifiers in this claim
              changed_references_hash = ReferenceAnalyzer.handle_changed_references(current_claim,
                                                                                    parent_claim, changed_references, added_references, removed_references, claim_key, index)
              ...
              changed_qualifiers_hash = QualifierAnalyzer.handle_changed_qualifiers(current_claim,
                                                                                    parent_claim, changed_qualifiers, added_qualifiers, removed_qualifiers, claim_key, index)
          ...
          parent_claims.each_with_index do |parent_claim, index|
            ...
            # check if there's any references or qualifiers in this claim
            removed_references = ReferenceAnalyzer.reference_updates(parent_claim, removed_references,
                                                                     claim_key, index)
            removed_qualifiers = QualifierAnalyzer.qualifier_updates(parent_claim, removed_qualifiers,
                                                                     claim_key, index)
          end
        else
          ...
          current_claims.each_index do |index|
            added_claims << { key: claim_key, index: index }
            # check if there's any references or qualifiers in this claim
            added_references = ReferenceAnalyzer.reference_updates(current_claims[index], added_references,
                                                                   claim_key, index)
            added_qualifiers = QualifierAnalyzer.qualifier_updates(current_claims[index], added_qualifiers,
                                                                   claim_key, index)
          end
        end
      end
      ...
      parent_content_claims.each do |claim_key, parent_claims|
        ...
        parent_claims.each_index do |index|
          ...
          removed_claims << { key: claim_key, index: index }
          # check if there's any references or qualifiers in this claim
          removed_references = ReferenceAnalyzer.reference_updates(parent_claims[index], removed_references,
                                                                   claim_key, index)
          removed_qualifiers = QualifierAnalyzer.qualifier_updates(parent_claims[index], removed_qualifiers,
                                                                   claim_key, index)
        end
      end
    end
