...
class LargeBatchesAnalyzer
    ...
    def self.handle_large_batches(revision_ids, batch_size)
        ...
        revision_ids.each_slice(batch_size) do |batch|
          ...
          parent_ids = parsed_contents.values.map { |data| data[:parentid] }.compact.reject(&:zero?)
        ...
        build_result(revision_contents, parent_contents)
      end
      
      def self.build_result(revision_contents, parent_contents)
        result = {}
